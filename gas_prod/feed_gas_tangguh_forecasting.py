...
def main():
    ...
    t1 = time.process_time()
    exec_time = "Execution time for ARIMAX model prediction: {}".format(format_timespan(t1-t0, max_units=3), True)
    logMessage(exec_time)
    ...
    # End time counting
    t1 = time.process_time()
    exec_time = "Execution time for SARIMAX model fitting: {}".format(format_timespan(t1-t0, max_units=3), True)
    logMessage(exec_time)
