CREATE OR REPLACE FUNCTION public.f_lng_cond_prod_ptbadak(lngplant character varying, startdate date, enddate date)
...
AS $function$ 
	...
	begin
		...
		_query = _query || 'cond.prod_date, cond.realization, ';
		_query = _query || '  CASE WHEN grf.grf IS NOT NULL THEN grf.grf ';
		_query = _query || '  WHEN adp.feed_gas IS NOT NULL and cond.prod_date > now() THEN adp.feed_gas ';
		...
		_query = _query || ' from lng_condensate_daily cond ';
		...
		then
			...
			_query = _query || ') ups ';
			_query = _query || ' on cond.prod_date = ups.unplanned_series ';
		end if;
		...
		then
			...
			_query = _query || ') ps ';
			_query = _query || ' on cond.prod_date = ps.planned_series ';
		end if;
		_query = _query || ' LEFT JOIN lng_target_badak_daily_feed_gas_grf grf ON cond.prod_date = grf.date';
		_query = _query || ' LEFT JOIN lng_target_badak_adp_monthly adp ON date_trunc(''MONTH'',cond.prod_date) = adp.prod_date';
		_query = _query || ' LEFT JOIN lng_feed_gas_daily fg ON cond.prod_date = fg.prod_date AND fg.lng_plant =''PT Badak''';
		...
		_query = _query || '   FROM lng_target_badak_daily_feed_gas_grf) grf_lastyear ON cond.prod_date = grf_lastyear.last_year_date';
		_query = _query || ' WHERE cond.lng_plant::text = ''PT Badak'''; 
		_query = _query || ' and cond.prod_date between ''' || startdate || ''' and ''' || enddate || ''''; 
		_query = _query || ' ORDER BY cond.prod_date DESC';
		--raise notice '%', _query;
		return QUERY 
		execute _query;
	end;
