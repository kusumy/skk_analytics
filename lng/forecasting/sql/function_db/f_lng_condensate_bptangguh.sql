CREATE OR REPLACE FUNCTION public.f_lng_condensate_bptangguh(lngplant character varying, startdate date, enddate date)
 RETURNS TABLE(lng_plant character varying, prod_date date, realization_prod double precision, target_wpnb double precision, is_unplanned_shutdown integer, is_planned_shutdown integer)
 LANGUAGE plpgsql
AS $function$ 
	...
	begin
		...
		_query = _query || 'prod.prod_date, prod.realization, prod.target_wpnb ';
		if(_upsCount > 0)
		...
		_query = _query || ' from lng_condensate_daily as prod ';
