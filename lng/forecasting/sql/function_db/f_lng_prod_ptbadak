CREATE OR REPLACE FUNCTION public.f_lng_prod_ptbadak(lngplant character varying, startdate date, enddate date)
...
AS $function$ 
	...
	begin
		...
		_query = _query || '  WHEN adp.feed_gas IS NOT NULL and lng.prod_date > now() THEN adp.feed_gas ';
